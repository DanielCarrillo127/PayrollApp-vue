    ...
    >
      ...
      <div class="action">
        <a :href="payroll.payrollUrl" target="_blank" class="ui button positive"
          >Descargar</a
        >
        <button class="ui button red" @click="deletePayroll(payroll.id, payroll.payrollUrl)">
          Eliminar
        </button>
      </div>
    </div>
...
import { deletePayrollFuntion } from "../../utils/firebase";
...
export default {
  ...
  setup(props) {
    ...
    const deletePayroll = async (id,url) => {
      try {
        await deletePayrollFuntion(id,url);
        props.getPayrolls();
      } catch (error) {
        console.log(error);
      }
